...
def Show_history(log_dic):
	print('\n\n=============历史测试节点信息==================')
	print("UE_ID\tTest_id\t\tPack_index\tReceiced Timer")
	for i in range(len(log_dic['ue_id'])):
		print("{0}\t{1}\t\t{2}\t\t{3}".format(log_dic['ue_id'][i], log_dic['test_id'][i], log_dic['pack_index'][i], log_dic['pack_time'][i]))
...
def parse_udp(data, addr):
	...
	log_dic = {'ue_id':[], 'log_file_list':[], 'test_id':[], 'pack_index':[], 'pack_time':[]}
	...
	if int(Pack_items['Pack_index']) == 0:	# Init pack
		...
		if Pack_items['UE_ID'] not in UE_ID_List:	# 全新的节点
			...
			Pack_Time_list.append(time.strftime('%Hh_%Mm_%Ss',time.localtime(time.time())))
			...
			for i in range(len(UE_ID_List)):
				if UE_ID_List[i] == Pack_items['UE_ID']:
					...
					Pack_Time_list[i] = time.strftime('%Hh_%Mm_%Ss',time.localtime(time.time()))
					break
			if i>=len(UE_ID_List):
